...
<script>
  ...
  departamento.addEventListener('change', async (event) => {
    ...
    if (optionSelected !== 'Escolher...') {
      const response = await axios.get(`http://192.168.0.108/intra-amcel-web/departamento/${optionSelected}/cargos`)
  ...
  form.addEventListener('submit', (event) => {
    ...
    if (departamento.options[departamento.selectedIndex].value === '0' ||
      ...
      axios({
        method: "POST",
        url: "http://192.168.0.108/intra-amcel-web/usuario/update",
        data: parameters
      }).then((response) => {
        if (response.data.success) alert('Salvo com sucesso!')
